...
class Servers(models.Model):
    ...
    def getFieldTemplate():
        ...
        with fieldTemplate.mq as mq:
            ...
            mq.server.fieldOptions = dict(
                label="RabbitMQ хост",
                initial="localhost",
                help_text="например mq.node.qos",
                required=False
            )
            ...
            mq.httpUrl.fieldOptions = dict(
                label="HTTP url",
                initial="http://localhost:15672",
                help_text="например<br/>http://localhost:15672",
                required=False
            )
            ...
            mq.amqpUrl.fieldOptions = dict(
                label="AMQP url",
                initial="amqp://guest:guest@localhost:5672/%2f",
                help_text="например<br/>amqp://guest:guest@localhost:5672/%2f",
                required=False
            )

            mq.heartbeatQueue.fieldClass = forms.CharField
            mq.heartbeatQueue.fieldOptions = dict(
                label="Очередь heartbeat",
                initial="heartbeat",
                help_text="Название очереди для<br/>сбора сообщений Q,<br/>по умолчанию heartbeat",
                required=False
            )

            mq.heartbeatAgentRequest.fieldClass = forms.CharField
            mq.heartbeatAgentRequest.fieldOptions = dict(
                label="Exchange для hb agent",
                initial="heartbeatAgentRequest",
                help_text="Название exchange для<br/>отправки сообщений агентам",
                required=False
            )

            mq.heartbeatAgentReply.fieldClass = forms.CharField
            mq.heartbeatAgentReply.fieldOptions = dict(
                label="Очередь для hb agent",
                initial="heartbeatAgentReply",
                help_text="Название очереди для<br/>приема результатов от агентов",
                required=False
            )
        ...
        with fieldTemplate.db as db:
            ...
            db.server.fieldOptions = dict(
                label="БД хост",
                initial="localhost",
                help_text="например db.node.qos",
                required=False
            )
            ...
            db.user.fieldOptions = dict(
                label="Пользователь",
                initial="qos",
                help_text="обычно qos",
                required=False
            )

            db.pwd.fieldClass = forms.CharField
            db.pwd.fieldOptions = dict(
                label="Пароль",
                initial="",
                help_text="Пароль БД",
                required=False,
        ...
        with fieldTemplate.fe as fe:
            ...
            fe.server.fieldOptions = dict(
                label="Frontend хост",
                initial="localhost",
                help_text="например fe.node.qos",
                required=False
            )
        ...
        with fieldTemplate.be as be:
            ...
            be.server.fieldOptions = dict(
                label="Backend хост",
                initial="localhost",
                help_text="например be.node.qos",
                required=False
            )
        ...
        with fieldTemplate.timeDB as timeDB:
            ...
            timeDB.httpUrl.fieldOptions = dict(
                label="Url для сохранения результатов",
                initial="",
                help_text="например http://localhost:8086/write?db=имяБД",
                required=False
            )
    ...
    qosguialarm = models.BooleanField(
        "Оповещать о событиях heartbeat в основном интерфейсе сервера",
        blank=False,
        null=False,
        editable=True,
        default=False
    ...
    def getConfigObject(self, decryptPwd=True):
        ...
        # {'fe': [{'server':'localhost'}]}
        return {nodeName:
                [{nodeKey: decryptPwdField(nodeKey, nodeValue[i])
                    for i, nodeKey in enumerate(template[nodeName].keys())
                    if i < len(nodeValue)}
                    for nodeValue in config.get(nodeName, [])
...
class ServerGroups(models.Model):
    ...
    usergroups = models.ManyToManyField(
        Group,
        blank=True,
        verbose_name="Доступна для группы пользователей",
        editable=True,
...
# If equals some of qos agentkeys - than heartbeat records and qos records are
# united in single box on gui
# if heartbeat agentkey not equals none of qos agentkey - than new box for
# heartbeat agent will be created
...
class Hosts(models.Model):
    ...
    # returns hosts aliases like {server:{host:{aliases set},host2:{aliases
    # set}},server2:...}
    def getAllAliases():
        res = {}
...
class ResultFormatters(models.Model):
    name = models.CharField("Имя обработчика", max_length=255,
                            blank=False, null=False, editable=True, default="")
...
class Items(models.Model):
    ...
    unit = models.CharField("Единица измерения", max_length=255,
                            blank=True, null=False, editable=True, default="")
    config = models.TextField(blank=False, null=False, default="",
...
class Triggers(models.Model):
    name = models.CharField("Имя триггера", max_length=255,
                            blank=False, null=False, editable=True, default="")
    config = models.TextField(blank=False, null=False, default="")
    durability = models.PositiveIntegerField(
        "Длительность (сек)",  blank=False, null=False, editable=True, default=0)
...
class TaskSets(models.Model):
    ...
    hosts = models.ManyToManyField(
        Hosts, verbose_name="Узлы сети", blank=True, editable=True)
    items = models.ManyToManyField(
        Items, verbose_name="Элементы данных", blank=True, editable=True)
    triggers = models.ManyToManyField(
        Triggers, verbose_name="Триггеры", blank=True, editable=True)
    enabled = models.BooleanField(
        "Включена", blank=False, null=False, editable=True, default=True)
    comment = models.CharField(
        "Комментарий", max_length=255, blank=True, null=False, editable=True)
    ...
    def getHeartbeatTasks(server, pollingPeriodSec):
        ...
        def getTaskConfig(sItemConfig, sHostConfig, itemKey):
            ...
            # string sHostConfig (optional) like {itemkey:{paramToOverride1:
            # valueToOverride1, paramToOverride2: ...}}
            ...
            if sItemConfig == "":
                ...
                try:
                    ...
                    raise Exception(
                        "неправильная конфигурация элемента данных")
            ...
            if sHostConfig != "":
                try:
                    ...
                    raise Exception(
                        "неправильная конфигурация узла сети " + str(e))

                if itemKey in hostConfig.keys():
                    # host parameters has priority and override same-named item
                    # patameters
        ...
        def getTaskFormat(formatStr):
            if formatStr is None:
                ...
                try:
                    ...
                    raise Exception(
                        "проверьте настройку обработки результата " + str(e))
        ...
        for task in tasks:
            ...
            try:
                taskConfig = getTaskConfig(
                    task.itemconfig, task.hostconfig, task.itemkey)
            except Exception as e:
                taskConfig = {}
