...
sampletypesForClipSearch = {sampletype_vdRadio,
                            sampletype_zipRadio, sampletype_vdTV}
...
def getDbConnection(dbConf):
    ...
    conString = "dbname='{1}' user='{2}' host='{0}' password='{3}' port='{4}'"\
        .format(dbHost, dbName, dbUser, dbPassword, dbPort)
...
def getTasks(dbConnection, defaultPeriod):
    ...
    # this field is none when module havent url property (all but MediaRecorder
    # and MediaStreamer)
    # LEFT JOIN on magenttask is used for "period" field
    # this field is none when module havent period property (i.e.
    # MediaRecorder and MediaStreamer)
    sql = """
    SELECT
    ...
    FROM
        magent as "agent", mmediaagentmodule as "module" LEFT JOIN (
                -- mmediaagentmodule_mstream is a link table from
                -- mmediaagentmodule to both mrecordedstream and mlivestream
                SELECT DISTINCT ON (id)
                    link2u.mmediaagentmodule_id as "id", m2u.url
                FROM mmediaagentmodule_mstream as "link2u",
                        -- mrecordedstream and mlivestream can be united
                        -- because their id's are not intersects
                        (
                        -- mrecordedstream contains urls for MediaRecorder
                        SELECT id, templatestreamurl as "url"
                        FROM mrecordedstream
                            UNION ALL
                        -- mlivestream contains urls for MediaStreamer
                        SELECT id, templateurl as "url"
                        FROM mlivestream
                        ) as "m2u"
    ...
    # incoming url like
    # "rtmpt://10.10.10.10:8077/qligentPlayer/stream_${properties.streamTaskId}"
...
def getOriginatorIdForAlertType(dbConnection, alertType):
    ...
    if not re.match("^[_A-Za-zа-яА-Я0-9/./-]+$", alertType):
        return ("Неверно задан тип аварийного сообщения," +
                " проверьте настройку qosAlertType", None)
    ...
    try:
        ...
        if len(rows) == 1:
            ...
            return (
                "Создайте хотя бы одно оповещение с типом " + alertType,
                None
            )
...
def getScheduledTaskKeys(dbConnection, timeStamp, zapas):
    ...
    with open(
            os.path.join(dirName, 'probe-config-defaults-v2.json'),
            encoding="UTF-8"
            ) as f:
        data = f.read()
    ...
    sql = """
        SELECT
            ...
            END as "parameters"
        FROM
            qos.task_schedule as "sc",
            qos.magent as "agent",
            qos.devices as "devices",
            qos.mass_media_monitoring as "services",
            --join sample types specified for this task and show them as list
            qos.task as "task" LEFT JOIN (
                SELECT task_id,type as "sampletype"
                FROM mass_media_sample_group
            ) as "samples" ON task.id=samples.task_id
        WHERE
            task.channel_id=services.id
            and task.probe_id=agent.id
            and task.id=sc.task_id
            and task.device_id=devices.id
            and devices.deleted=false
            and {0}>sc.begin_time
            and {1}<least(sc.end_time,sc.until)
        GROUP BY
            agent.entity_key,
            task.channel_id,
            devices.type,
    ...
    res = set()
    for row in rows:
        agentKey, channelId, device, profileId, * \
            configCodes, sampleTypes, taskParameters = row
        ...
        # find allowed modules for non-"MATCH" devices
            ...
            # confignames contains level keys in probeConfigServices structure:
            ...
            # {"AD": false, "RF": false, "TS": true, "AUDIO": false,
            # "VIDEO": true, "BITRATE": false, ...}
            ...
            if configCodes[0] == 0:
                allowedProfiles = set(
                    {pName: pAllowed
                        for pName, pAllowed in profiles[profileId].items()
                        if pAllowed
                     }
                )
            # service_type is "SELF"
                ...
                allowedProfiles = set(
                    {pName: pAllowed
                        for pName, pAllowed
                        in taskParameters['profiles'].items()
                        if pAllowed
                     }
                )
            ...
            # scan probeConfigServices structure and get allowed module profile
            # names wich are allowed to run now
            # moduleProfiles like
            # {"TR101290","MpegTSStatisticsIPTVControlModule_STREAM"}
            ...
            for nextLevelName in configNames:
                ...
                if 'profile' in levelData:
                    ...
                    if not allowedProfiles:
                        ...
                        tmp = {
                            pName: pValue
                            for pName, pValue in allModuleProfiles.items()
                            if pName in allowedProfiles
                        }
            ...
            # TODO: scan probeConfigDevices structure and check that device
            # supports moduleProfiles and signal types.
            # Exclude moduleProfiles that are not supported by device

            pass

            # scan probeConfigModules structure and convert moduleProfiles
            # names to module name with dot and perfix
            # moduleNamePerfix like
            # {"TR101290.","MpegTSStatisticsIPTVControlModule.",
            # "SelfMonitor.SMCPU_"}
        ...
        # convert moduleNames to full-specified taskKeys like
        #   "MyAgent.SelfMonitor.SMCPU_2223"
        # add taskKeys to res
        sampleTypes = set(sampleTypes)
        for moduleNameAndPerfix in moduleNames:
            # add ImageSearch and ClipSearch tasks only if samples of
            # corresponding type are specified
