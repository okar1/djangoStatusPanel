...
def sendRegisterMessage(server, routingKeys):
    ...
    msgHeaders = {
        "__TypeId__": "com.tecomgroup.qos.communication.message.ServerStarted"}
    ...
    # raise exception only if all mq's are down, so message sending is
    # impossible
    if mqConf is None:
    ...
    channel.queue_bind(queue=queueName, exchange=exchangeName,
                       routing_key="server.agent.register")
...
def pollDb(dbConfig, serverName, vServerErrors, oldTasks, dbQueryFn, *dbQueryArgs):
    ...
    if tasksToPoll is None:
        if dbConfigured and oldTasks is not None:
            ...
            # if settings was changed and no any db host is specified now -
            # then not prolongate oldTasks
            tasksToPoll = {}
...
def getMqConf(mqConfig, serverName, vServerErrors):
    ...
    # mqAmqpConnection if first working mq was found and res is corresponding
    # mqConf
    res = None
    for mqConf in mqConfig:
        try:
            con = pika.BlockingConnection(
                pika.URLParameters(mqConf['amqpUrl']))
        except Exception as e:
            errors.add(str(e))
        else:
            if res is None:
...
def pollMQ(serverName, mqConsumerId, vServerErrors, vTasksToPoll):
    ...
    if serverError is not None:
        ...
        vServerErrors.update(formatErrors(
            {serverError['error']}, serverName, pollName))
    ...
    vServerErrors.update(formatErrors(errors, serverName, pollName))
    vServerErrors.update(formatErrors(
        delayedErrors, serverName, 'RabbitMqTaskNotInDB', delayedError=True))
...
def subSendHeartBeatTasks(mqConf, serverName, tasksToPoll, serverErrors):
    ...
    errors = heartbeatAgent.sendHeartBeatTasks(
        con, tasksToPoll, mqConf['heartbeatAgentRequest'], True)
    serverErrors.update(formatErrors(errors, serverName, "hbSender"))
    con.close()


# receive heartbeat tasks request from rabbitmq queue
def subReceiveHeartBeatTasks(mqConf, serverName, tasksToPoll, serverErrors,
                             oldTasks):
    con = pika.BlockingConnection(pika.URLParameters(mqConf['amqpUrl']))
    errors = heartbeatAgent.receiveHeartBeatTasks(
        con, tasksToPoll, mqConf['heartbeatAgentReply'], True)
    ...
    for taskKey, task in tasksToPoll.items():
        ...
        if task.get("module", None) == 'heartbeat' and task['enabled']:
            ...
            if "value" in task.keys():
                ...
                # this causes removing value and "no value received" error for
                # "empty" composite tasks
                # 2. allow MediaRecorderControl to return {}
                # this causes to remove task from list (if node has no
                # mediaRecorder tasks)
                if task['value'] == {} and \
                        task.get('config', {}).get('item', None) != \
                ...
                if type(value) == dict:
                    ...
                    for childTaskKey, childTaskValue in value.items():
                        # scenario 1:
                        # tasks like MediarecorderControl are applying to
                        # another tasks like "MediaRecorder"
                        # For such tasks childTaskKey is key of "target" and it
                        # already presents in tasksToPoll
                        # so, we update target with "parentkey", "value", and
                        # "timestamp" fields and leave another keys
                        # unchanged (incl ModuleName)
                        # scenario 2:
                        # if task not applying to another tasks - we just take
                        # copy of parent task
                        # and update this copy with child task's "parentkey",
                        # "value", and "timestamp"
                        childTask = tasksToPoll.get(childTaskKey, task.copy())
                        # tasks with "parentkey" property set are the children
                        # tasks
                        # ex. cpu load of core #1 #2 ... #24 are the children
                        # tasks of "cpu load"
                ...
                # check if there are early decomposed (children) tasks
                #    in oldTasks with
                ...
                if oldChildrenTasks:
                    ...
                    newChildrenTasks = {k: task.copy()
                                        for k in oldChildrenTasks.keys()}
                    # add some parameters from oldChildrenTass to
                    # NewChildrenTasks
...
def useOldParameters(vTasksToPoll, oldTasks):
    for taskKey, task in vTasksToPoll.items():
        if taskKey in oldTasks.keys():
            if 'stateChangeTimeStamp' not in task.keys() and \
                    ('stateChangeTimeStamp' in oldTasks[taskKey].keys()):
                task['stateChangeTimeStamp'] = \
                    oldTasks[taskKey]['stateChangeTimeStamp']
            if 'timeStamp' not in task.keys() and \
                    ('timeStamp' in oldTasks[taskKey].keys()):
            ...
            if 'alarmTimeStamps' not in task.keys() and \
                    ('alarmTimeStamps' in oldTasks[taskKey].keys()):
                task['alarmTimeStamps'] = oldTasks[taskKey]['alarmTimeStamps']
            if 'value' not in task.keys() and \
                    ('value' in oldTasks[taskKey].keys()):
                task['value'] = oldTasks[taskKey]['value']
            if 'unit' not in task.keys() and \
                    ('unit' in oldTasks[taskKey].keys()):
                task['unit'] = oldTasks[taskKey]['unit']
            if 'parentkey' not in task.keys() and \
                    ('parentkey' in oldTasks[taskKey].keys()):
                task['parentkey'] = oldTasks[taskKey]['parentkey']


def markTasks(tasksToPoll,
              oldTasks,
              pollStartTimeStamp,
              appStartTimeStamp,
              pollingPeriodSec,
    ...
    def markHeartBeatTask(taskKey, task):
        ...
        if expResCount is not None:
            ...
            factResCount = sum(
                [1 for t in tasksToPoll.values() if isTaskToCount(t)])
            if expResCount != factResCount:
                task['error'] = "в настройках задано результатов: " + \
                    str(expResCount) + " фактически получено: " + \
                    str(factResCount)
                return
            # end if
        # end if
        ...
        # if after applying we got a value (not [None, False, empty dict, empty
        #    list, etc ])
        ...
        alarmTimeStamps = {k: v for k,
                           v in alarmTimeStamps.items() if k in alarmsFired}
        ...
        # error if one of alarms raised out of specified duration
        # (show first raised alarm)
        ...
        for aKey, aTimeStamp in alarmTimeStamps.items():
            ...
            if aKey in alarms.keys():
                if abs(aDuration) >= \
                        alarms[aKey].get('duration', 0) * pollingPeriodSec:
                    task['error'] = aKey
                    return
    # end sub
    ...
    for taskKey, task in tasksToPoll.items():
        ...
        # timestamp when task state changed from enabled to disabled and vise
        # versa
        ...
        if taskEnabled:
            if timeStamp:
                ...
                # so, make extended check for heartbeat tasks (alarms,
                # resultcount etc)
                if task.get('error', None) is None and \
                        task['module'] in modulesReturningValue:
            ...
            if task.get('error', None) is None:
                ...
                if abs(pollStartTimeStamp - timeStamp) > aDuration:
                    task['error'] = \
                        'задача не присылает данные длительное время'
                    continue
        else:
            # since task is disabled - remove timestamp and value to prevent
            # including it in oldTasks
            # in this case alarm will not fired imidiately when task will
            # became enabled in future
            ...
            if pollStartTimeStamp - stateChangeTimeStamp > aDuration:
                # check 1: task is disabled, but schedule support is enabled
                # and task is scheduled now
                if 'scheduled' in task.keys() and task['scheduled'] is True and \
                   'qosEnabled' in task.keys() and task['qosEnabled'] is False:
                    task['error'] = "задача запланирована, но не работает"
                    continue

                # check2: ...but for last aDuration  seconds we still receive
                # data (but shoud not)
    ...
    if tasksToPoll is not None:
        for taskKey, task in tasksToPoll.items():
            ...
            taskData.update({key: task[key]
                             for key in
                             # agentName not included - because it already
                             # presents in box name
                             ["style", "agentKey", "timeStamp", "enabled",
                                 "unit", "value", "error", "itemName"]
                             if key in task.keys()})
            ...
            for errorId, error in serverErrors.items():
                ...
                if not error.get('enabled', True):
                    ...
                    if errorId in delayedServerErrors:
                        ...
                        error['enabled'] = True
                    # in any case - add to delayed to alarm was fired at next
                    # poll
...
def pollResultSort(vPollResult):
    ...
    vPollResult.sort(key=lambda v: (
        "0" if v.get("servertask", False) and "error" in v.keys() else
        "1" if "error" in v.keys() else
        "3" if v.get('enabled', True) is False else
        "2") +
        v['pollServer'] + " " + v['name']
    )


def qosGuiAlarm(tasksToPoll,
                oldTasks,
                serverName,
                serverDb,
                mqConf,
    ...
    if e is None:
        ...
        for taskKey, task in tasksToPoll.items():
            ...
            action = "ACTIVATE" if task.get(
                'style', None) == 'rem' else "CLEAR"

            # 0 - not published
            # 1 - published activate
            # 2 - published clear
            alarmPublishStatus = tasksToPoll[
                taskKey].get('alarmPublishStatus', 0)
            needPublish = False
            ...
            if needPublish:
                ...
                channel.basic_publish(
                    ...
                    properties=pika.BasicProperties(
                        ...
                        expiration="86400000",
                        headers={
                            '__TypeId__': 'com.tecomgroup.qos.communication.' +
...
def commitPollResult(tasksToPoll,
                     serverName,
                     vServerErrors,
                     timeDbConfig,
                     vPollResult):
...
# if some tasksToPoll name found in alias list - rename such task to hostname,
# that owns alias
...
def applyHostAliases(allServerAliases, server, vTasks):
    ...
    for task in vTasks.values():
        ...
        for hostName, aliases in allHostAliases.items():
            # change "native" name in qos task to hostname that has
            # corresponding alias
...
def disableQosTasks(allServerHostEnabled,
                    serverDB,
                    pollingPeriodSec,
                    pollStartTimeStamp,
                    serverName,
                    vTasksToPoll,
                    vServerErrors):
    # request qos db for channel status. Which chnnels are actve now and which
    # are not
    scheduledTaskKeys = set()
    # zapas is seconds count after channel becomes active and before it becomes
    # inactive
    # in "zapas" period channelSchedule still retutns active=false
    # (in really it already=true)
    ...
    if serverDB:
        try:
            ...
            scheduledTaskKeys = qosDb.getScheduledTaskKeys(
                serverDB, pollStartTimeStamp, zapas)
        except Exception as e:
            vServerErrors.update(formatErrors(
                [str(e)], serverName, "channelSchedule"))
...
# run heartbeat agent locally in server context to process tasks with
# localroutingkey
# no amqp is used to sent/receive local tasks
def runAgentLocally(tasksToPoll, serverName, vServerErrors):
    localTasks = {k: v for k, v in tasksToPoll.items() if v.get(
        "agentKey", None) == localRoutingKey}
    try:
...
def formatValue(v, format, oldV):
    ...
    def _do1value1format(v, format):
        ...
        # true if python-true value received
            ...
            raise Exception(
                "поле item не задано либо некорректно." +
...
def formatTasksValues(tasksToPoll):
    ...
    for taskKey, task in tasksToPoll.items():
        ...
        if task['enabled'] and \
            ...
            try:
                # assert that last value is not need when formatting, only when
                # alerting
                task['value'] = formatValue(
                    task['value'], task['format'], None)
            except Exception as e:
...
# some heartbeat tasks can have "applyTo" field, wich means this task takes
# another tasks parameters as argument
# ex. task "mediaRecorderControl" is applying to tasks "mediaRecorder"
# here we process applyTo field from hb task settings and substitute names
# with actual task id and parameters
def fillApplyTo(tasksToPoll, vHbTasks, serverName, vServerErrors):
    ...
    for hbtask in vHbTasks.values():
        ...
        if hbtask['enabled'] and \
                (applyTo is not None) and \
                item in settiings.keys():
            ...
            #
            # 2) non local mode for MediaRecorderControl (slow, use only for
            # small amount of sbk on server)
            # For this config NO hbAgent on cbk is required.
            # MediaRecorder must be configured for 1 heartbeatAgent
            # (local or remote)
            ...
            for taskKey, taskData in tasksToPoll.items():
                ...
                if localMode:
                    # local hb agent must have same name and key like in qos
                    # task
                    # for local task was routed correctly
                    if hbtask['agentName'].lower() != \
                            taskData['agentName'].lower() or \
                       hbtask['agentKey'].lower() != \
                            taskData['agentKey'].lower():
                        continue

                targetHostFound = True

                # assert that target found if we found any corresponding tasks
                # (even disabled)
                # this disables error "no tasks found" in case if tasks are
                # disabled now
                if not taskData['enabled']:
                    continue

                newApplyTo.update({taskKey: {
                    f: "127.0.0.1" if localMode and f == "serviceIp" else
                    taskData.get(f, None)
                    for f in useFields
                }})
            ...
            if not targetHostFound:
                errors.add(
                    "На {0} ({1}) нет задач qos. Задача {3} остановлена.".
                    format(hbtask['agentName'],
                           hbtask['agentKey'],
                           target,
                           item)
                )
    ...
    # disable target (mediaRecorder) tasks that was not applied from any
    # mediaRecorderControl
    for taskKey, taskData in tasksToPoll.items():
    ...
    vServerErrors.update(formatErrors(
        errors, serverName, 'MediaRecorderControl'))
