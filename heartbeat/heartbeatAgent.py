...
# pyasn1 help
# http://www.red-bean.com/doc/python-pyasn1/pyasn1-tutorial.html#1.1.7
...
isTestEnv = True if len(sys.argv) == 2 and sys.argv[
    0] == 'manage.py' and sys.argv[1] == 'runserver' else False

presets = {
    "smLsiRaid": {
        "item": "snmptable",
        "oid": "1.3.6.1.4.1.10876.100.1.11",
        "indexcol": 7,
        "datacol": 27
    },
    "smChasis": {
    ...
    "windowsFirewallStatus": {
        "item": "shell",
        "command": "cmd /C chcp 65001 && netsh Advfirewall show allprofiles",
        "utf8": True,
        "timeout": 3,
        "include": [r"""
            (?sx)
    ...
    "windowsNetworkType": {
        "item": "shell",
        "command": "powershell -c Get-NetConnectionProfile",
        "utf8": True,
        "timeout": 6,
        "include": [r"""
            (?sx)
    ...
    # 51.140.127.197  128.138.141.172  2 u   23   64    1   75.455  117.537
    # 0.000
    "ntpSyncAgoSec": {
        "item": "shell",
        "command": "'c:\\Program Files (x86)\\NTP\\bin\\ntpq.exe' -p",
        "utf8": True,
        "timeout": 20,
        "include": [r"""
            (?x)
    ...
    "intelRaid": {
        "item": "shell",
        "command": "c:\\monitoring\\rstcli64.exe -I -v",
        "utf8": True,
        "timeout": 3,
        "include": [r"""
            (?sx)
...
def sendHeartBeatTasks(mqAmqpConnection, tasksToPoll, sendToExchange, serverMode=False):
    ...
    try:
        channel.exchange_declare(
            exchange=sendToExchange, exchange_type='topic', durable=True)
...
def receiveHeartBeatTasks(mqAmqpConnection, tasksToPoll, receiveFromQueue, serverMode=False):
    ...
    try:
        channel.queue_declare(queue=receiveFromQueue, durable=True, arguments={
    ...
    try:
        channel.exchange_declare(
            exchange=receiveFromQueue, exchange_type='topic', durable=True)
    except Exception as e:
        errors.add(str(e))
        return errors

    try:
        channel.queue_bind(queue=receiveFromQueue,
                           exchange=receiveFromQueue, routing_key="#")
    except Exception as e:
        errors.add(str(e))
        return errors
    ...
    while True:
        ...
        try:
            getOk, * \
                mqMessage = channel.basic_get(receiveFromQueue, no_ack=True)
        except Exception as e:
            errors.add(str(e))
            continue
        if getOk:
    ...
    for msg in mqMessages:
        ...
        if serverMode:
            if headers.get('protocolversion', 0) < agentProtocolVersion:
                errors.add(
                    'Версия heartbeatAgent устарела. ' +
                    'Обновите heartbeatAgent на ' + msg[0].routing_key
                )
                continue

            if taskKey not in tasksToPoll.keys():
                errors.add(
                    'Ошибка обработки сообщения: неверный ключ ' + taskKey)
                continue

            try:
                stringTimeStamp = headers['timestamp']
                taskTimeStamp = datetime.strptime(
                    stringTimeStamp, timeStampFormat)
                tasksToPoll[taskKey]['timeStamp'] = taskTimeStamp
            except Exception as e:
                errors.add(
                    "Ошибка обработки сообщения: неверная метка времени.")
                continue

            error = headers.get('error', None)

            # filter fields when reseiving at server side
            tasksToPoll[taskKey].update(
                {k: v for k, v in msgBody.items() if k in ['value', ]})
            tasksToPoll[taskKey]['unit'] = taskUnit
            if error is not None:
                tasksToPoll[taskKey]['error'] = error
        else:
            # filter fields when reseiving at agent side
            tasksToPoll[taskKey] = {'module': 'heartbeat', 'agentKey': msg[
                0].routing_key, 'unit': taskUnit}
            tasksToPoll[taskKey].update(
                {k: v for k, v in msgBody.items() if k in ['config', ]})
...
def updateShovels(mqConf, errors):
    ...
    rqUrl = "{0}parameters/shovel/%2f/{1}-request".format(
        mqApiUrl, mqConf['hbRoutingKey'])
    rqShovelData = {"value": {
        ...
        .format(
            ...
            mqConf['pwd'],
            mqConf['hbServer'],
            mqConf['hbServerVhost'],
            randomNumber
        ),
        "src-exchange-key": mqConf['hbRoutingKey'],
        "dest-uri": "amqp://{0}:{1}@localhost"
        .format(
            mqConf['user'],
            mqConf['pwd']
        ),
    ...
    try:
        req = requests.put(rqUrl, auth=(
            mqConf['user'], mqConf['pwd']), json=rqShovelData)
    except Exception as e:
        errors.update(str(e))
        return
    ...
    replUrl = "{0}parameters/shovel/%2f/{1}-reply".format(
        mqApiUrl, mqConf['hbRoutingKey'])
    ...
    req = requests.put(replUrl, auth=(
        mqConf['user'], mqConf['pwd']), json=replShovelData)
...
def checkParameters(param, pattern):
    ...
    par2remove = set({k: None for k in param.keys()
    ...
    for paramKey, paramOpts in pattern.items():
        ...
        if isMandatory and (paramKey not in param.keys()):
            raise Exception("не указан параметр " + paramKey +
            ...
            if paramValue is not None:
                ...
                if (type(paramType) == list and
                        (type(paramValue) not in paramType)) or \
                   (type(paramType) != list and
                        (type(paramValue) != paramType)):
                    raise Exception(
                        "тип параметра " + paramKey +
                        " задан неверно. Проверьте настройки программы")
            else:
...
def _snmpFormatValue(v):
    ...
    if _type is NoSuchObject:
    ...
    elif sum([issubclass(_type, baseType)
              for baseType in
              [TimeTicks, Integer, Integer32, Gauge32, Counter32, Counter64, Unsigned32]
              ]):
        res = int(v)
    elif sum([issubclass(_type, baseType)
              for baseType in
              [ObjectIdentity, ObjectIdentifier, IpAddress]]):
        ...
        pp = v.prettyPrint()
        s = str(v)
        # sometimes prettyPrint returns hex values instead of string because
        # of trailing 00 byte
...
# when gettable=true get full snmp table like dict
# {indexRow1:[col1,col2,col3],...}
...
def taskSnmpTableValue(include, oid, host, port, readcommunity, indexcol, datacol):
    ...
    if tableId not in tableDict.keys():
        tableList = list(taskSnmpGet(
            oid, host, port, readcommunity, getTable=True).values())
        if tableList:
            if len(tableList[0]) - 1 < max(indexcol, datacol):
                # print(tableList)
                raise Exception(
                    "значение indexcol или datacol в настройках больше, чем количество столбцов в таблице snmp")
    ...
    if table:
        ...
        raise Exception(
            "Не удалось загрузить таблицу SNMP. Проверьте настройки ПО и доступность оборудования.")
...
def _wmiOhmTable():
    ...
    # +item.Name
    return {
        ((item.Identifier).replace('/', '.')[1:]):
        item.Value
        for item in q
        }


# identifier of openHardwareMonitor table value like
...
def taskOhwTableValue(include):
    ...
    if table:
        ...
        raise Exception(
            "Не удалось загрузить данные Open Hardware Monitor. Проверьте работу ПО.")

    if len(res) == 0:
        raise Exception(
            "Не удалось найти значение в таблице Open Hardware Monitor")
    # elif len(res)==1:
    #     return res[list(res.keys())[0]]
    else:
        return res
...
# parses string with every regex in include. Return key:value dict for regex
# with 2 groups and a list of values for regex with 1 group
# all regexes in include must have same format (all 1-group or all 2-group)
            ...
            for item in match:
                if type(item) == str and item != '':
                    ...
                    if len(item) == 2:
                        ...
                        raise Exception(
                            "regex должен возвращать строго 1 или 2 группы. Проверьте настройки ПО.")
                else:
...
# 1 group result is processed as taskvalue, taskkey will be generated
# automatically
# If "utf8" flag is set - decode console stdout as cp65001 else cp866
#   (it is usefull with system commands on russian windows locale.
# All output messages became english)
...
def taskShellTableValue(command, include, utf8, timeout):
    ...
    if command not in commandDict.keys():
        ...
        if commandList is None:
            # when file name or path contain spaces -
            # full path must be included in single or double quotes
            ...
            if command[0] == '"' or command[0] == "'":
                ...
                if command.find(quoteSymbol, 1) == -1:
                    raise Exception(
                        "Если путь к программе содержит пробелы - используйте кавычки")
                exeNamePath = command.split(quoteSymbol)[1]
                # 2 symbols for open and close quotes
                paramString = command[len(exeNamePath) + 2:]
                commandList = [exeNamePath] + paramString.split(" ")
                # remove empty spaces in case of -param1<space1><space2>-param2
                commandList = [v for v in commandList if v != '']
            else:
        ...
        # print(commandResult)
        commandResult = commandResult.decode(
            "cp65001") if utf8 else commandResult.decode("cp866")
...
# applyto like {'nn02.MediaRecorder.8007': {'serviceIp': '87.245.203.38',
# 'servicePort':80}, }
def taskMediaRecorderControl(applyTo):
    ...
    def _do1task(taskKey, taskData):
        ...
        url = 'http://{0}:{1}/qligentPlayer/streams/{2}'.format(
            taskData['serviceIp'], taskData['servicePort'], taskId)
        try:
        ...
        if not datesList:
            template = [
                "/streams/{0}/{1}/([\d_]*)/".format(agentKeyAndModule, taskId)]
            url = 'http://{0}:{1}/qligentPlayer/streams/{2}/{3}'.format(
                taskData['serviceIp'],
    ...
    # this means, that this node hasn't corresponding MediaRecorder tasks to
    # apply
    ...
    # 10 threads parallel
    futureDict = {
        futures.ThreadPoolExecutor(10).submit(_do1task, taskKey, taskData):
        (taskKey, taskData)
        for taskKey, taskData in applyTo.items()
    }
    res = {}
...
def taskUptime():
    ...
    pattern = [r"(\d{14})\.\d+\+\d+"]
    lastBootTimeList = taskShellTableValue(
    ...
    uptimeDays = round((datetime.now() - datetime.strptime(
        lastBootTimeList[0], bootTimeStampFormat)).total_seconds() / 84400, 4)

    # first 30 minutes after startup query last kernel power event
    if uptimeDays < 0.02:
        pattern = ["Event ID: (\d+)"]
        # wevtutil qe System /c:1 /rd:true /f:text
        # /q:"*[System[Provider[@Name='Microsoft-Windows-Kernel-Power'] and
        # (EventID=109 or EventID=41)]]"
        commandList = \
            ["wevtutil", "qe", "System", "/c:1", "/rd:true", "/f:text",
            "/q:*[System[Provider[@Name='Microsoft-Windows-Kernel-Power'] and (EventID=109 or EventID=41)]]"]
        lastBootEventCode = taskShellTableValue(
            command=commandList, include=pattern, utf8=False, timeout=5)
        if len(lastBootEventCode) != 1:
            raise Exception("Получено неверное значение: " +
                            str(lastBootEventCode))
...
# taskstoPoll like
# {Trikolor_NN.Heartbeat.1:
#   {"module":"heartbeat",
#   'type': 'qtype1',
#   'agentKey': 'Trikolor_NN',
#   'config': {'header': 'task1'}
# }}
...
def processHeartBeatTasks(tasksToPoll):
    ...
    for taskKey, task in tasksToPoll.items():
        ...
        if item == "shell":
            task[
                'error'] = "Непосредственный запуск модуля shell запрещен в целях безопасности"
            continue
        ...
        if preset is not None:
            ...
            if preset.get('item', None) == 'shell':
                # for security reasons passing external shell commands is not
                # allowed
        ...
        try:
            if item == "ohwtable":
                task['value'] = taskOhwTableValue(**checkParameters(
                    taskConfig, {
                        "include": {"type": list,
                                    "mandatory": True},
                                }
        ...
        if ('error' not in task.keys()) and ('value' in task.keys()):
            ...
            if type(value) == dict:
                ...
                if item in ["MediaRecorderControl"]:
                    ...
                    task['value'] = {k: v for k,
                                     v in value.items() if v is not None}
                else:
                    # items that returns "short" task keys - append parent task
                    # key
                    task['value'] = {taskKey + "." + k: v for k,
                                     v in value.items() if v is not None}
        # endif
...
def agentStart():
    ...
    errors.update(receiveHeartBeatTasks(
        mqAmqpConnection, tasksToPoll, receiveFromQueue))
    processHeartBeatTasks(tasksToPoll)
    errors.update(sendHeartBeatTasks(
        mqAmqpConnection, tasksToPoll, sendToExchange))
    # print("sleep 10 sec")
    time.sleep(10)
