...
def threadPoll():
    ...
    while True:
        ...
        for server in Servers.objects.all():
            ...
            # oldtsks are used to get timestamp if it absent in current
            # taskstopoll
            ...
            # serverConfig['mq']=[mqConf] -> mqConf (select first working
            # mqconf)
            mqConf = subs.getMqConf(
                serverConfig['mq'], server.name, serverErrors)
            # opt["maxMsgTotal"]
            ...
            if mqConf and (tasksToPoll or hbTasks):
                ...
                # opens mq consumer for this server. If it alredy opened - do
                # nothing
                MqQosResultConsumers.createUpdateConsumers(
                    {
                        mqConsumerId:
                        (mqConf['amqpUrl'], mqConf['heartbeatQueue'])
                    }
                )

                # polling RabbitMQ (download messages from consumer), add
                # "idleTime" to tasksToPoll
                subs.pollMQ(server.name, mqConsumerId,
                            serverErrors, tasksToPoll)

                # send heartbeat tasks request to rabbitmq exchange
                subs.subSendHeartBeatTasks(
                    mqConf, server.name, tasksToPoll, serverErrors)

                # messages with local routing key are not actually send to
                # rabbitMQ
                ...
                subs.subReceiveHeartBeatTasks(
                    mqConf, server.name, tasksToPoll, serverErrors, oldTasks)

                # debug mode for heartbeatAgent: disable "sendHeartBeatTasks"
                # and "receiveHeartBeatTasks" via amqp
                # and call "processHeartBeatTasks" directly
                # from . import heartbeatAgent
                # heartbeatAgent.processHeartBeatTasks(tasksToPoll)

                # apply result formatters to tasks in tasksToPoll that contains
                # a value
        ...
        if isTestEnv:
            print("--------------------------end poll (",
                  threadPoll.pollTimeStamp - pollStartTimeStamp, " sec)")
            print("--------------------------------------------")
