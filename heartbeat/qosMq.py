...
def pollRabbitMQ(
        ...
        # if current poll not contain timestamp - then use timestamp from prev.
        # poll
        oldTasks={"taskKey": {}},
):
    ...
    if amqp in mqProto:
        ...
        httpLink = "http://{0}:{1}/api/queues/%2f/{2}/get".format(
            rabbitToPoll["server"], rabbitToPoll["port"], heartbeatQueue)
    ...
    while len(mqMessages) > 0:
        ...
        if amqp in mqProto:
            ...
            try:
                req = requests.post(
                    httpLink,
                    auth=(rabbitToPoll["user"],
                          rabbitToPoll["pwd"]),
                    json={
                        "count": messagesPerRequest,
                        "requeue": False,
                        "encoding": "auto"})
            except Exception as e:
                errors += [str(e)]
            ...
            if type(mqMessages) != list:
                errors += ["Не могу обработать данные: " +
                           str(type(mqMessages))]
                return
    ...
    for taskKey, task in tasks.items():
        if taskKey in oldTasks.keys():
            ...
            if ('alarmPublishStatus' not in tasks.keys()) and \
