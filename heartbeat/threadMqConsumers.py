    ...
    def _consumerLoop(self):
        ...
        def onChannelOpen(newChannel):
            ...
            channel.queue_declare(
                ...
                exclusive=False,
                auto_delete=False,
                arguments=self.queueArgs,
                callback=onQueueDeclared
            )
        ...
        def checkConsumerStop():
            ...
            if not connection.is_open:
                connection.add_timeout(
                    self.stopConsumerCheckIntervalSec, checkConsumerStop)
            else:
        ...
        try:
            ...
            while True:
                connection = pika.SelectConnection(
                    pika.URLParameters(self.amqpUrl), onConnected)
                connection.add_timeout(
                    self.stopConsumerCheckIntervalSec, checkConsumerStop)
                ...
                if self.error is not None:
                    print('connection lost (' +
                          self.error +
                          '). Try to reconnect in 5 sec for',
                          self.consumerName
                          )
                else:
                    print('connection lost. Try to reconnect in 5 sec for',
                          self.consumerName)
