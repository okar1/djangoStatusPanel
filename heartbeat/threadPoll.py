    ...
    while True:
        ...
        for server in Servers.objects.all():
            ...
            if tasksToPoll is not None:
                ...
                # now tasksToPoll like {taskKey: {"agentKey":"aaa",
                # "period":10} }}
                qosMq.pollRabbitMQ(errors=errors, tasks=tasksToPoll,
                                   rabbits=serverConfig['mq'], opt=opt,
                                   oldTasks=oldTasks)
                # now tasksToPoll like {taskKey: {"agentKey":"aaa",
                # "period":10,"idleTime":timedelta} }}
            ...
            if tasksToPoll is not None and \
                    server.qosguialarm and serverDbConfig:
                ...
                e, originatorId = qosDb.getOriginatorIdForAlertType(
                    dbConf=serverDbConfig, alertType=opt['qosAlertType'])
                if e is None:
            ...
            if len(serverErrors) > 0:
                pollResult += [{
                    "id": server.name,
                    "name": server.name,
                    "pollServer": server.name,
                    "error": "Ошибки при опросе сервера " + server.name,
                    "data": serverErrors,
                    }]
            ...
            resultNoErrors = [{"id": server.name + '.' + key,
                               "name": key,
                               "data": taskData,
                               "pollServer": server.name}
                              for key, taskData in agents.items()
